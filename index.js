...
const ProcessWatchdog = require('./process-watchdog');
...
function addWatchdogOfExistingProcesses() {
  ...
  pm2.list(
     ...
     * @param {pm2.ProcessDescription[]} apps
     */
    function (err, apps) {
      if (err) {
...
pm2.launchBus(function (err, bus) {
  ...
  bus.on(
     ...
     */
    function (data) {
      if (data.process.axm_options.isModule) {
        return;
      } // Ignore PM2 modules
...
function startOrRestartWatchdog(pm2Env) {
  const processScript = moduleConfig[pm2Env.name];
  ...
  watchdog = new ProcessWatchdog(pm2Env, {
    processScript,
    checkingInterval:
      parseInt(moduleConfig[pm2Env.name + '/checking_interval'] || moduleConfig.checking_interval) || 10,
    failsToRestart: parseInt(moduleConfig[pm2Env.name + '/fails_to_restart'] || moduleConfig.fails_to_restart) || 3,
  });
